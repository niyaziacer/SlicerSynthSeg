...
# Helper function for TensorFlow 2.x compatibility
def get_shape_list(tensor):
    """
    Get shape as list, compatible with TensorFlow 2.x.
    Handles both Tensor objects and lists.
    """
    if hasattr(tensor, 'shape'):
        # It's a Tensor
        return tensor.shape.as_list()
    elif isinstance(tensor, (list, tuple)):
        # It's already a list or tuple
        return list(tensor)
    else:
...
def ae(nb_features,
    ...
    if single_model:
        ...
        in_input_shape = get_shape_list(enc_model.output)[1:]
        in_model = None
    ...
    if single_model:
        ...
        in_input_shape = get_shape_list(mid_ae_model.output)[1:]
        in_model = None
    dec_model = conv_dec(nb_features,
                         in_input_shape,
                         nb_levels,
...
def conv_dec(nb_features,
    ...
    if input_model is None:
        ...
        input_shape = get_shape_list(last_tensor)[1:]
...
def single_ae(enc_size,
    ...
    if input_model is None:
        ...
        last_tensor = input_model.output
        input_shape = get_shape_list(last_tensor)[1:]
    input_nb_feats = get_shape_list(last_tensor)[-1]
    ...
    if ae_type == 'dense':
        ...
        if list(enc_size)[:-1] != list(input_shape)[:-1] and \
            ...
            zf = [enc_size[:-1][f] / get_shape_list(last_tensor)[1:-1][f] for f in range(len(enc_size) - 1)]
    ...
    if do_vae:
        ...
        if ae_type == 'dense':
            ...
            if list(enc_size)[:-1] != list(input_shape)[:-1] and \
                ...
                name = '%s_ae_sigma_enc' % prefix
                last_tensor = convL(get_shape_list(pre_enc_layer)[-1], conv_size, name=name, **conv_kwargs)(
                    pre_enc_layer)
    ...
    if ae_type == 'dense':
        ...
        if list(enc_size)[:-1] != list(input_shape)[:-1] and \
            ...
            zf = [get_shape_list(last_tensor)[1:-1][f] / enc_size[:-1][f] for f in range(len(enc_size) - 1)]
            last_tensor = layers.Resize(zoom_factor=zf, name=name)(last_tensor)
